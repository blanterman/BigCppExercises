...
#include "stdafx.h"
#include "P1110_Onetime.h"
#include "P1110_Daily.h"
#include "P1110_Weekly.h"
#include "P1110_Monthly.h"
#include <iostream>
#include <vector>
...
int main()
{
	...
	*/
	...
	bool enterNew = false;
	bool readApp = false;
	bool saveAppBook = false;
	bool loadAppBook = false;
	int year;
	int month;
	int day;
	int time;
	...
	while (cont)
	{
		// Ask the user if they would like to add an appointment 
		// or read appointment by date
		// or save appointments to a file
		// or load appointments from a file
		// or quit
		...
		// User selected to add new appointments
		while (enterNew)
		{
			while (!valid)
			{
		...
		// User selected to read appointments from book by date
		while (readApp)
		{
		...
		// User selected to save appointments to file
		if (saveAppBook)
		{
			// Initialize the output stream
			std::ofstream bookFile;
			// Open file
			bookFile.open("P1110_AppBook.txt");

			// Check to see if the file opened save all appointments if open
			if (bookFile.is_open())
			{
				for (int i = 0; i < apps.size(); i++)
				{
					apps[i]->save(bookFile);
				}
			}
			// if the file didn't open then report an error
			else
			{
				std::cout << "Error saving appointments\n\n";
			}

			// close the file
			bookFile.close();

			// reset variable to return to main menu
			saveAppBook = false;
		}

		// User selected to load appointments
		if (loadAppBook)
		{
			// initialized vars
			std::string line;
			...
			// if the file opened, parse it line by line and fill appointments
			if (bookFile.is_open())
			{
				// get the next line checking for the appointment type keywords
				while (std::getline(bookFile, line))
				{
					// When an appointment type keyword is found, create the appointment
					// and add it to the appoinntment vector
					if (line == "Onetime")
					{
						Onetime* temp = new Onetime();
						temp->load(bookFile);
						apps.push_back(temp);
					}
					if (line == "Monthly")
			...
			{
				std::cout << "Error loading appointments\n\n";
			}
