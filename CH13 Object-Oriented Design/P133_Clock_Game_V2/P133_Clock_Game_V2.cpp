/** Exercise P13.3. In the clock game program, we assigned the play_round 
function to the Game class. That choice was somewhat arbitrary. Modify the 
clock program so that the Player class is responsible for play_round.

Bryson Lanterman
10/4/24
Notes: Change was straight forward: Move member function, move dependant 
private functions, and change the way variables are being called i.e. from 
getters to variable references.
    I added a function to display the level and the score. That was a nice 
change.
	I would like to add capability for a maximum score based on a player's name. 
Maybe use a file that is saved and read every time.
*/
#include "ccc_win.h"
#include "ccc_time.h"
#include <cstdlib>
...
public:
	...
	*/
	...
	/**
		Plays a round with up to two tries
	*/
	void play_round();
	/**
		Adds level and score to clock board
	*/
	void display_level_score();
	/**
...
void Player::play_round()
{
	...
	if (time_to_guess.seconds_from(guess) != 0)
	{
		...
		text = "Sorry, " + name + ". That is incorrect. Correct time: " + time_string + ".";
	}
	else
	{
		text = "Congratulations, " + name + "! That is correct.";
		increment_score();
	}
	cwin.clear();
	clk.draw();
...
void Player::display_level_score()
{
	cwin << Message(Point(-7, -8), "Level: " + std::to_string(level));
	cwin << Message(Point(4, -8), "Score: " + std::to_string(score));
}
...
std::string Player::format_time(Time t)
{
