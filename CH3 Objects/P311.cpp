...
// This program uses linear algebra techniques to find the intersection of two lines

using namespace std;

int ccc_win_main()
{
	// Gets 2 points from the user to define line 1
	Point a = cwin.get_mouse("Select endpoints for first line");
	cwin << a; // user feedback
	Point b = cwin.get_mouse("Select endpoints for first line");
	cwin << b << Line(a, b); // user feedback

	// Gets 2 points from the user to define line 2
	Point c = cwin.get_mouse("Select endpoints for second line");
	cwin << c; // user feedback
	Point d = cwin.get_mouse("Select endpoints for second line");
	...
	{
		Message m1(Point(-8.0, 8.0), "Lines are parallel");
		cwin << m1;
