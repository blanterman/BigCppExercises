...
/* Exercise P8.6. The arc sine function can be computed from the arc tangent 
function, according to the formula

arcsin(x) = arctan(x / sqrt(1 - (x * x)))

use that expression as an oracle to test that your arc sine function works 
correctly. Test both functions with 100 random inputs.
*/

/**
	Computes the arc sine of an input using the function 
	arcsin(x) = arctan(x / sqrt(1 - (x * x)))
	@param x double input between -1 and 1
	@return arc sine of x.
*/
double arcsin_oracle(double x)
{
	if (x < -1.0 || x > 1.0)
	{
		cout << "Invalid input\n";
...
int main()
{
	...
	for(int i = 0; i < 100; i++)
	{
		...
		val1 = arcsin(x);
		val2 = arcsin_oracle(x);
		if (!approx_equal(val1, val2))
			cout << "Test Failed. ";
		else
			cout << "Test Passed. ";
		cout << "arcsin(" << x << ") = " << val1 << "\n";
